...
| LDAP_FILTER  | string  | `"(|(sAMAccountName=username)(mail=username))"` or `"(|(uid=username)(mail=username))"`| LDAP Filter. Choose the filter based on your LDAP configuration. the value "username" is used as a placeholder and will be replaced with the actual username in authentication function.|
